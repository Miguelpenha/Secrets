...
export const ThemeProvider: FC = ({ children }) => {
    ...
    async function loadTheme() {
        const themeName = await AsyncStorage.getItem('@secrets:theme') as (IthemeType | null)
        
        if (themeName) {
            setThemeName(themeName)
        } else {
            setThemeName(Appearance.getColorScheme() || 'light')
            AsyncStorage.setItem('@secrets:theme', Appearance.getColorScheme() || 'light')
        }
    }
    ...
    function mutateTheme(themeNameMutate?: IthemeType) {
        ...
        AsyncStorage.setItem('@secrets:theme', themeNameSelect)
